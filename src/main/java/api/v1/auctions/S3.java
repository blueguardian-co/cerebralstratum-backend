...
@Path("/s3")
public class S3 extends S3CommonResource {
    @Inject
    S3Client s3;
    ...
    public Response uploadFile(S3FormData formData) throws Exception {
    ...
    public List<S3FileObject> listFiles() {
        ...
        return s3.listObjects(listRequest).contents().stream()
                .map(S3FileObject::from)
                .sorted(Comparator.comparing(S3FileObject::getObjectKey))
