...
`DeviceEventBroadcaster` is the single point of Kafka consumption for device telemetry — one `@Incoming` consumer per topic (`device.location`, `device.status`, `device.canbus`), fanned out in-process via `BroadcastProcessor` and filtered per-device or per-device-set (`locationUpdatesFor`/`statusUpdatesFor`/`canBusUpdatesFor`, overloaded for a single `UUID` or a `Set<UUID>`). Two transports consume it (ADR-0010):
- `DeviceServerSentEvents` — SSE, one connection per device per component (`/api/v1/devices/by-id/{device_uuid}/location|status|canbus`), the original web client transport.
- `DeviceMultiplexedServerSentEvents` — SSE, one connection per component covering many devices (`/api/v1/devices/locations|statuses|canbuses?devices=<uuid>&devices=<uuid>...`), added to avoid the browser per-host connection limit and server-side connection overhead a client hits watching more than a couple of devices via the per-device transport. Each requested device is access-checked once at connect time via the same UMA path as `DeviceServerSentEvents` (device grants live only in Keycloak, ADR-0005 — there's no cheap in-process substitute), and unauthorized devices are silently dropped rather than failing the whole request. Authorization is front-loaded, not re-checked per event, so a device whose access is revoked mid-connection keeps streaming until the client reconnects — clients should reconnect periodically (every 5-10 min) to bound that window.
