...
- [CSPROD-183](https://youtrack.blueguardian.co/issue/CSPROD-183): once real location data flowed through this ADR's REST/SSE endpoints (`LocationResource`, `DeviceServerSentEvents`), `quarkus-rest-jackson`'s build-time serializer codegen — which has no visibility into the runtime-registered `GeometryJacksonModule` — fell back to a reflection-based serializer for `Point` and recursed forever over JTS's self-referential geometry getters (`StackOverflowError`). Fixed by annotating the `coordinates` fields directly with `@JsonSerialize`/`@JsonDeserialize`. Worth flagging for any future ADR introducing a new geometry-typed REST/SSE field (e.g. Privacy Zones' zone polygon, ADR-0008) — the same fix will be needed there too.
- [CSPROD-182](https://youtrack.blueguardian.co/issue/CSPROD-182): `cerebralstratum-frontend`'s map never rendered the markers this ADR's SSE stream was correctly delivering — two independent frontend bugs (coordinate order, and an auth-token-refresh dependency that tore down every EventSource connection every ~30-70s). Backend-side transport was working correctly throughout; this was purely a frontend consumption issue.
